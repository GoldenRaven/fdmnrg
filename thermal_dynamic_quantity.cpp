...
double inner_energy()
{
        ...
        for (int n=n0;n<N_max-1;n++){
                sum_E=0;
                for (int l=num_kept;l<num_basis[n];l++){
                        sum_E=sum_E+exp_z[n][l-num_kept]*(eigen[n][l].eig_val_relat+E_GS[n])*pow(Lambda,-1.0*(eigen[n][l].n-1)/2.0);
                }
        ...
        for (int n=N_max-1;n<N_max;n++){
                sum_E=0;
                for (int l=0;l<num_basis[n];l++){
                        sum_E=sum_E+exp_z[n][l]*(eigen[n][l].eig_val_relat+E_GS[n])*pow(Lambda,-1.0*(eigen[n][l].n-1)/2.0);
                }
                sumn_E=sumn_E+pow(4,N_max-1-n)*sum_E;
        }
...
double entropy_kB()
{
        ...
        for (int n=n0;n<N_max-1;n++){
                ...
                for (int l=num_kept;l<num_basis[n];l++){
                        sum_E=sum_E+exp_z[n][l-num_kept]*(eigen[n][l].eig_val_relat+E_GS[n])*pow(Lambda,-1.0*(eigen[n][l].n-1)/2.0);
                        sum_Z=sum_Z+exp(-1.0*Beta*((eigen[n][l].eig_val_relat+E_GS[n])*pow(Lambda,-1.0*(eigen[n][l].n-1)/2.0)-E_GS[N_max-1]*pow(Lambda,-1.0*(eigen[N_max-1][0].n-1)/2.0)));
        ...
        lnZ=-1.0*Beta*E_GS[N_max-1]*pow(Lambda,-1.0*(eigen[N_max-1][0].n-1)/2.0)+log(sumn_Z);
